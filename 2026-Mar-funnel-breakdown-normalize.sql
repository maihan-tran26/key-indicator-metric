...
WHEN DATE_TRUNC('month', DATE_FROM_PARTS(interaction_date_yy, interaction_date_mm, 1))
    ...
    WHEN DATE_TRUNC('month', DATE_FROM_PARTS(interaction_date_yy, interaction_date_mm, 1))
        ...
        , CASE 
                ...
                WHEN top_funnels ='FA_Other' AND funnel = 'FA_OMF' then '7. Full-App Flow'
                ...
                WHEN top_funnels ='Logged_NativeApp' AND funnel = 'Mobile App' then '7. Full-App Flow'
                ...
                WHEN top_funnels ='Other' AND funnel = 'NCPQ_Email' then '7. Full-App Flow'
                WHEN top_funnels ='Other' AND funnel = 'EA_Remarketing' then '3. Remarketing'
                WHEN top_funnels ='Other' AND funnel = 'Others' then 'Other'
                WHEN top_funnels ='Search_B_FA' AND funnel = 'FA_OMF' then '7. Full-App Flow'
                WHEN top_funnels ='Search_B_PQ' AND funnel = 'PQ_OMF' then '1. SEM'
                WHEN top_funnels ='Search_Direct_FA' AND funnel = 'FA_OMF' then '7. Full-App Flow'
                WHEN top_funnels ='Search_Direct_PQ' AND funnel = 'PQ_OMF' then '3. Search_Direct'
                WHEN top_funnels ='Search_NB_FA' AND funnel = 'FA_OMF' then '7. Full-App Flow'
    ...
    WHERE hard_submit_ssn IS NOT NULL 
    GROUP BY 1,2,3,4,5,6
    ORDER BY 1,2
), current_risk_distro AS (
    SELECT 
        funnel_type 
        , final_category
        , hard_rsk_lvl_grp
        , SUM(hard_submits) OVER(PARTITION BY funnel_type, final_category, hard_rsk_lvl_grp) AS hps 
        , SUM(hard_submits) OVER(PARTITION BY funnel_type, final_category) AS total_hps 
        , hps/total_hps AS risk_distro 
    FROM C1 
    WHERE 
        record_of_interest = '1. last_month'
    ORDER BY 1,2,3
), hps_by_risk_band AS (
    SELECT
        funnel_type 
        ...
        , record_of_interest
        , hard_rsk_lvl_grp
        , SUM(hard_submits) OVER(PARTITION BY funnel_type, final_category, record_of_interest, hard_rsk_lvl_grp) AS hard_submits
        , SUM(visitors)OVER(PARTITION BY funnel_type, final_category,record_of_interest) AS total_visitors
        , hard_submits/total_visitors AS hard_submit_rate 
    FROM C1 
    ORDER BY 1,2,3,4
), norm_v1 AS (
    SELECT 
        hbrb.funnel_type 
        , hbrb.final_category 
        , hbrb.record_of_interest 
        , hbrb.hard_rsk_lvl_grp
        , hard_submit_rate 
        , risk_distro
        , hard_submit_rate * risk_distro AS norm_hard_submit_rate 
    FROM hps_by_risk_band hbrb 
    LEFT JOIN current_risk_distro crd 
    ON hbrb.funnel_type = crd.funnel_type AND hbrb.final_Category = crd.final_Category AND hbrb.hard_rsk_lvl_grp = crd.hard_rsk_lvl_grp 
), final_norm AS (
    SELECT 
        funnel_type 
        , final_category
        , record_of_interest
        , SUM(norm_hard_submit_rate) AS norm_visitor_to_hps 
    ...
    SELECT
        funnel_type 
        , final_category
        , MAX(CASE WHEN metric_rank = 1 then norm_visitor_to_hps END) AS current_v_to_hps 
        , MAX(CASE WHEN metric_rank = 2 then norm_visitor_to_hps END) AS last_month_v_to_hps 
        , MAX(CASE WHEN metric_rank = 3 then norm_visitor_to_hps END) AS last_year_v_to_hps  
    FROM final_norm 
    GROUP BY 1,2
    ORDER BY 1,2
)
...
SELECT * FROM C3 
WHERE funnel_type NOT ILIKE '%oam%'
