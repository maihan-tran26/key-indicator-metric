...
SELECT 
    Interaction_date_YY
    , interaction_date_mm
    , COUNT(DISTINCT CASE WHEN soft_submit_flag = 1 then person END) AS sps 
    , COUNT(DISTINCT CASE WHEN soft_approval_flag = 1 then person END) AS spa 
    , COUNT(DISTINCT CASE WHEN hard_submit_flag = 1 then person END) AS hps 
    , COUNT(DISTINCT CASE WHEN credit_approved_flag = 1 then person END) AS hpa 
FROM bdm_marketing.DACR_ACQ_FUNNEL_COMBINED_PBI_V2 
WHERE 
    FUNNEL IN ('PQ_Email','PQ_OMF','PQ_OAM')
    and interaction_date_yy IN (2024,2025)
GROUP BY 1,2
ORDER BY 1,2 
